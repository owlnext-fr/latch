...
- [ ] **Step 6 :** `configuration.mdx` — *source : ENVIRONMENT, `.env.example`.* Référence **exhaustive des 17 clés**, par groupe : **obligatoires prod (fail-secure)** [`ADMIN_USER`, `ADMIN_PASS`, `DEPLOY_TOKEN`, `LATCH_PUBLIC_BASE_URL`, `SESSION_SECRET`, `UNLOCK_COOKIE_SECRET`] ; **réglages** [`LATCH_UNLOCK_TTL_DAYS`, `LATCH_UNLOCK_RL_IP_BURST`, `LATCH_UNLOCK_RL_IP_REPLENISH_PER_SEC`, `LATCH_UNLOCK_RL_SLUG_BURST`, `LATCH_UNLOCK_RL_SLUG_PERIOD_SECS`, `LATCH_BODY_LIMIT`, `LATCH_STORAGE_ROOT`, `LATCH_SPA_DIST`, `DATABASE_URL`, `PORT`, `LATCH_IMAGE_TAG`]. Chaque clé : rôle, défaut, obligatoire prod oui/non, génération (`openssl rand -hex 32`).
